        ...
        {education.map((edu, index) => (
          <FadeIn key={index} delay={index * 150}>
            <div className="group relative glass p-8 rounded-2xl hover:border-indigo-500/20 transition-all duration-300 card-hover h-full">
              ...
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-100 dark:from-indigo-500/20 to-cyan-100 dark:to-cyan-500/20 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
              ...
              <div className="flex items-center gap-2 text-sm font-mono text-slate-400 dark:text-slate-500 mb-3">
                <Calendar size={14} className="text-indigo-600 dark:text-indigo-400" />
                {edu.duration}
              </div>

              {edu.details && (
                <div className="flex items-center gap-2 mt-4 px-3 py-2 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 w-fit">
                  <Award size={16} className="text-emerald-600 dark:text-emerald-400" />
                  <span className="text-sm text-emerald-700 dark:text-emerald-300 font-medium">{edu.details}</span>
                </div>
              )}
            </div>
          </FadeIn>
